...
def _build_title_nature_hint(content_nature: str, benefit_type: str) -> str:
    """content_nature 기반 제목 작성 전략 힌트 (비콜라보 소재 전용)."""
...
def build_title_prompt(brand: str, promotion_content: str, target: str, remarks: str = "", collab_pair: str = "", content_nature: str = "", benefit_type: str = "") -> str:
    target_ctx = {"여성": "여성 고객 대상", "남성": "남성 고객 대상"}.get(target, "전체 고객 대상")
    _remarks_section = f"\n## 마케터 메모 (우선 반영)\n{remarks.strip()}" if remarks and remarks.strip() else ""
    _nature_hint_section = _build_title_nature_hint(content_nature, benefit_type)
...
## 소재 정보
- 브랜드: {brand or "정보 없음"}
- 프로모션 내용: {promotion_content or "정보 없음"}{_remarks_section}{_collab_section}
...
- 훅 필요 시:      감성·호기심 유발 문구      예) 빵처럼 맛있는 쉐이크 발견{_nature_hint_section}
...
def _build_content_strategy_section(content_nature: str, benefit_type: str) -> str:
    """content_nature × benefit_type 조합 기반 본문 작성 전략 힌트."""
    hints = []
...
## ⚠️ 콜라보 소재 — 본문 중복 금지
제목이 이미 "{collab_pair}" 콜라보를 표시합니다.
본문에서 **{names} 이름을 절대 반복하지 마세요**.
본문은 발매·혜택·기간·긴급성만 서술합니다.

✅ 올바른 예: 제목 "{collab_pair}" → 본문 "(광고) 무신사 단독 한정 발매"
❌ 잘못된 예: 제목 "{collab_pair}" → 본문 "(광고) {collab_pair} 콜라보, 무신사 단독 발매" ← 쌍 반복"""

    _strategy_section = _build_content_strategy_section(content_nature, benefit_type)
...
- 콘텐츠 유형: {content_type or "정보 없음"}{_remarks_section}{collab_section}{forbidden_kws_section}{_strategy_section}
...
def build_content_fix_prompt(
    ...
    return f"""당신은 무신사 앱푸시 메시지 검수 전문가입니다. {target_ctx}

아래 본문에서 규칙 위반이 발견됐습니다. 위반 사항만 수정하여 올바른 본문을 다시 작성하세요.
...
## 프로모션 내용 (참고)
{promotion_content}{forbidden_kws_section}
...
6. **제목-본문 분리**: 제목의 핵심 단어가 본문에 그대로 반복되지 않는가? 두 문장이 자연스럽게 이어지는가?
