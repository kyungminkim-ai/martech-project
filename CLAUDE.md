...
| 앱푸시 에이전트 상세 설정 조회 | `ai-copywriting/match-push-agent/CLAUDE.md` |
...
| 에이전트 | 경로 | 상태 |
|---------|------|------|
| match-push-agent | `ai-copywriting/match-push-agent/` | ✅ 운영 중 |
| email-agent | `ai-copywriting/email-agent/` | 🔲 미구현 |
| sms-agent | `ai-copywriting/sms-agent/` | 🔲 미구현 |
...
1. `ai-copywriting/{channel}-agent/` 디렉터리 생성
2. 4-phase pipeline 구조 유지 (선별 → 생성 → 검수 → Red Team)
3. `ai-copywriting/{channel}-agent/.claude/skills/{channel}-campaign/SKILL.md` 등록
4. 이 파일 라우팅 테이블 & 에이전트 디렉터리 업데이트
5. 루트 `README.md` 에이전트 현황 업데이트

참고 구현: `ai-copywriting/match-push-agent/`
